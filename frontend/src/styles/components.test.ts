 ...
 *  - кнопки `.btn` имеют правила `:hover`/`:active`/`:disabled` и `min-height: 36px`;
...
describe('Структурные правила компонентов в global.css', () => {
  describe('Кнопки (Req 6.1, 6.3, 6.4)', () => {
    it('базовая кнопка задаёт min-height: 36px', () => {
      const body = ruleBody(css, '.btn');
      expect(declValue(body, 'min-height')).toBe('36px');
    });
  ...
  describe('Модальные окна (Req 11.1, 11.6)', () => {
    it('окно использует тень всплывающего слоя --shadow-popover (Req 11.6)', () => {
      expect(
        /\.modal\s*(?:,\s*[^{}]+)?\{[^{}]*box-shadow\s*:\s*var\(--shadow-popover\)/s.test(css),
      ).toBe(true);
    });
