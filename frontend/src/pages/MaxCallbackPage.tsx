...
import { MAX_OAUTH_STATE_KEY, type MaxOAuthPurpose } from '@/lib/max-oauth';
 ...
 */
export function MaxCallbackPage({ purpose }: { purpose: MaxOAuthPurpose }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const failKey = purpose === 'login' ? 'login.maxFailed' : 'profile.max.failed';
    const stateMismatchKey =
      purpose === 'login' ? 'login.maxStateMismatch' : 'profile.max.stateMismatch';
    const noCodeKey = purpose === 'login' ? 'login.maxNoCode' : 'profile.max.noCode';
